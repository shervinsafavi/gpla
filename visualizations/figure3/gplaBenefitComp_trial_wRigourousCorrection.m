    ...
    for ksigma = 1:length(Sigma)
        ...
        for ksim = 1:nSim
            for ktrial = 1:nTrial
                ...
                % PLVshNrmlz(:,:,ksim) = (PLV(:,:,ksim)+lfp*spkTimes)/sqrt(T*nTrial*mean(rate));
                Nspk(:,ksim) = Nspk(:,ksim)+sum(spkTimes,1)';
                lfpCorr(:,:,ksim) = lfpCorr(:,:,ksim)+lfp*lfp';
            end
        end
        PLV=PLV/T/nTrial;
        PLVshNrmlz = PLV / sqrt(mean(rate));
        % PLVcentered = %
        varPLV = var(real(PLV), [], 3) + var(imag(PLV), [], 3);
        varPLVshNrmlz = var(real(PLVshNrmlz), [], 3) + var(imag(PLVshNrmlz), [], 3);
        ...
        for ksim = 1:nSim
            ...
            couplingMatrix(:,:, ksim)= inv(M)*PLV(:,:,ksim)*diag(1./sqrt(rate));
