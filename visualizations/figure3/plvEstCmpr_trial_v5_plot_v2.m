...
% 
dph2 = loglog(lfpAmpNoise_sigmas, couplingSNR.plaEst(kEst_start,:)', ...
             'linewidth', 1, 'color','k');
hold all
dph1 = loglog(lfpAmpNoise_sigmas, couplingSNR.gplaEst(kEst_start,:)', ...
             'linewidth', 3, 'color','k');
...
trialNumsTrimmed = trialNums(kEst_start:end);

[lh1, icons1] = legend(ph, strcat('Trial number = ',cellstr(num2str(trialNumsTrimmed'))), 'location','northeast');
...
% new vals:
displaceFigureStuff(lh1, [.022 .045 0 0])
