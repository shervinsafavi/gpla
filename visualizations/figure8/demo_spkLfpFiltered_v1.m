...
% tr = 1 : 700;
verticalOffset = 2.5;
% slw = 1; % spike linewidth
% llw = 1.5; % LFP line width
slw = .5; % spike linewidth
llw = 1; % LFP line width
sl = .4;

iu = 1; 
% iTr = 1;

continuousData = real(lfpAnalSig_allSepFreq{ifreq}(iu, tr, iTr));
% lfpLikeSig(iu, tr, iTr);
eventData = sparseSPT{iTr}(iu, tr);
...
iu = 10; 
continuousData = real(lfpAnalSig_allSepFreq{ifreq}(iu, tr, iTr));
% continuousData = lfp(iDataset).v(iu, tr, iTr);
% lfpLikeSig(iu, tr, iTr);
eventData = sparseSPT{iTr}(iu, tr);
