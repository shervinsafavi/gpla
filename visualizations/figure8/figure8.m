% in v13 we have circular color bar
...
for k = 1 : numel(freqBandsLab)
    freqBandsLabHz{k} = [freqBandsLab{k}, ' Hz'];
end
...
vcrd.all = [...
    vcrd.c1x vcrd.r1y; ... % A
    vcrd.c2x_r1r2 vcrd.r1y; ... % B
    vcrd.c1x vcrd.r2y; ... % C
    vcrd.c2x_r1r2 vcrd.r2y; ... % D
    vcrd.c1x vcrd.r3y; ... % E
    vcrd.c2x_r3r4 vcrd.r3y; ... % F
    vcrd.c1x vcrd.r4y; ... % G
    vcrd.c2x_r3r4 vcrd.r4y; ... % H
    vcrd.c1x .42; ... % I
    vcrd.c1x .29; ... % J
    vcrd.c1x .17      % K
           ];

vcrd.labels = {'A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'J', 'K'};


% for k = 1 : size(vcrd.all, 1)
%     annotation('textbox',[vcrd.all(k,1) vcrd.all(k,2) 0 0], ...
%                'String', vcrd.labels{k}, ...
%                'FontSize', vc.f5.sblfz, ... 
%                'LineWidth',1, ...
%                'EdgeColor', 1*[1 1 1], ...
...
% set(th, 'fontsize', vc.f5.fs_spkLfpText);
% displaceFigureStuff(th, [NaN .3 NaN]);

% % txt = '$$LFP(t) = \sum_{k = 1}^{5} w_k O_k(t)$$';   
% % text(-100,-1,txt, 'Interpreter', 'latex')
...
displaceFigureStuff(sbh, [tos -.02 -tos NaN]);
% run calcLfpPhaseGradient_v1.m
% run calcLfpPhaseGradient_v2.m

run phaseModulusPlots_v0.m
...
% clf
% inset([.87 .25 .095 .095]);
inset([.87 .235 .095 .095]);
axis square
% circ_collbar([0.1 1])
tickFontSize = vc.f5.fs_text;
