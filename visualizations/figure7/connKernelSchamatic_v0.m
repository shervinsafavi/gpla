...
linewidth = 6.5; % desired linewidth
tau = .3;
...
vc.f13.schfs = 14;
vc.f13.schfsArrow = 12;

xline(0, 'k:', 'linewidth', 3)
text(0, -.1, 'Center', 'HorizontalAlignment', 'center', 'FontWeight','normal', 'fontsize', vc.f13.schfs)
text(-7.5, 1.8, 'Weak recurrence', 'HorizontalAlignment', 'center', 'FontWeight','normal', 'fontsize', vc.f13.schfs)
text(-7.5, .8, 'Strong recurrence', 'HorizontalAlignment', 'center', 'FontWeight','normal', 'fontsize', vc.f13.schfs)
...
% x = 1:.1:10; % x data. Assumed to be increasing
% y = exp(-x); % y data
% N = 100; % number of colors. Assumed to be greater than size of x
% cmap = hsv(N); % colormap, with N colors
% linewidth = 4.5; % desired linewidth
% xi = x(1)+linspace(0,1,N+1)*x(end); % interpolated x values
% yi = interp1(x,y,xi); % interpolated y values
% hold on
