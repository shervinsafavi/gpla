% in v3 we have additional statisital analysis for the linear fit 
...
for klfp = selLFP
    ...
    for kPlt = 1 : numel(plotTypeIds)
        ...
        % end
        if kPlt == 1
            displaceFigureStuff(sbh, [-hos2 vos_svs2 -sbhPos(3)/4 -sbhPos(4)/4])
        elseif kPlt == 2
            displaceFigureStuff(sbh, [-hos3 vos_svs2 -sbhPos(3)/4 -sbhPos(4)/4])
        elseif kPlt == 3
            displaceFigureStuff(sbh, [-1.7*hos2 vos_svs2 -sbhPos(3)/4 -sbhPos(4)/4])
        end
        for ksim = simList
            ...
            switch ksim
              ...
              case 1
                ...
                load(fullfile(pds.ldat, 'biophysWaveSimul',['neuralWaveStats1JLargeCenter' lfpExt{klfp}]),'lfpVec','spkVec','gPLV','allStat','map','globalDynamicsParams','SF','Nbin')

              case 2
                load(fullfile(pds.ldat, 'biophysWaveSimul', ...
            ...
            switch kplot
              case 1
                ...
                % lh = legend('Weak recurrence', 'Strong recurrence', 'location','northoutside')
                % set(lh, 'box', 'off')
              ...
              case 4
                ...
                ylim([-50 100])
                ph(ksim) = plot(x,x*res.Coefficients.Estimate(2)+ ...
                ...
                % % stataliz for later
                linFitNeuralFiled.pVal(ksim)= res.Coefficients.pValue(2);
                linFitNeuralFiled.slope(ksim) = res.Coefficients.Estimate(2);
...
%%
% lh = legend([ph(1), ph(2)], 'Weak recurrence', 'Strong recurrence', 'location','north')
% set(lh, 'box', 'off')
% displaceFigureStuff(lh, [NaN .03 NaN NaN])
% % displaceFigureStuff(lh, [.12 .0055 NaN NaN])

lh = legend([ph(1), ph(2)], 'Weak recurrent inhibition', 'Strong recurrent inhibition', 'location','north')
set(lh, 'box', 'off')
displaceFigureStuff(lh, [.06 .043 NaN NaN])
% displaceFigureStuff(lh, [.12 .0055 NaN NaN])

%%  check some stats for the paper
linFitNeuralFiled.pVal < 10e-4
