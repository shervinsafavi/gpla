...
% filter lfp for each trial and channel, plus subsequent phase extraction
% signalParams.nTr = size(lfp,2);
% signalParams.nCh = size(lfp,3);
...
% subplot(nR,nC, [3 4])
% coi = find(Sig.cell_label == 3 | Sig.cell_label == 4);
% tspt = full(spkTrains{itr}(coi, tr));
% plot_eventRaster(tspt, {'color','k', 'linewidth',1})
% xlim([0 ns])
...
% coi = find(Sig.cell_label == 3);
% % coi = coi(1:30);
% tspt = full(spkTrains{itr}(coi, tr));
% plot_eventRaster(tspt, {'color','k', 'linewidth',1})
% title('CA1 pyr cell spikes')
% box off; axis off
% xlim([0 ns])

% subplot(nR,nC, 4)
% coi = find(Sig.cell_label == 4);
% tspt = full(spkTrains{itr}(coi, tr));
% plot_eventRaster(tspt, {'color','k', 'linewidth',.5})
% title('CA1 interneurons spikes')
% box off; axis off
% xlim([0 ns])
